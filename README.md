...
door to open, guests to punch, a bar with four drinks and a bottle to carry off.

## Holding things (the PlaceHolder API)

Anything in a level can be picked up and carried. In a script, name it and say so:
...
The name is the one the level gives the object - the same name the Creator lists down the side,
so a script can be written by copying what is on screen. A scene can also set `"holdable": true`
on the object directly; the script is the shape most mods want, because it lets a level decide
*when* something becomes carryable.

| In a script | What it does |
|---|---|
| `hold_item("Bottle")` | The player can pick that object up |
| `give_item("Bottle")` | Puts it straight into their hands |
| `drop_item()` | Puts down whatever they are holding |
| `held_item()` | The object in their hand, or `None` |

**Playing**: walk up to it, **E** picks it up, **Q** puts it back down in front of you. A held
item hangs off the player's right hand bone with its **base inside the fist**, so model an item
standing on its bottom the way a bottle stands on a bar - that base is what the hand closes
around. Tune the grip with `-Droastengine.holdInset=` (how far into the fist, in metres) and
point it at another rig's bone with `-Droastengine.holdBone=` (`hand_r` by default).

`player_hold.glb` next to the game is the pose for carrying something, loaded the same way as
`player_idle.glb` and `player_punch.glb`: the rig, one clip, exported from the same skeleton.
Without it the player carries things with their idle pose, which works but looks unbothered.
Walking always uses the walk cycle - the item is attached to the hand either way, so it travels
with them.

**The bag.** On its own the engine gives the player one pair of hands and one item. The
**PlaceHolder API** ([packs/placeholder-api/](packs/placeholder-api/)) gives them slots:

```json
{ "name": "PlaceHolder API", "version": "1.0.0", "slots": 5 }
```

Install and enable it and a row of five slots appears along the bottom of the screen; **1** to
**5** pick which one is in the hand, and picking something up goes into the hand when it is
empty and into the bag when it is not. The number lives in the mod, so a bigger bag is a new
version of the API rather than a new build of the game (the engine allows up to 9). Switch the
mod off and the player is back to two hands and one item.

## Gun Arena

**Gun Arena** ([mods/gun-arena/](mods/gun-arena/), generated by `tools/generate_gun_arena.py`) is
a team shooter for two or more players: red against blue, two maps (**Warehouse** and
**Courtyard**), six guns, first to 10 kills or the most after five minutes. Offline it is a
practice range with dummies to shoot.

In the lobby, vote for a map, click **Loadout** to pick which guns you carry, then ready up; the
match starts once everyone is ready. Carrying more than one gun needs the **PlaceHolder API**
enabled - the loadout fills its slots, and **1**-**5** switch between them. In a match: click to
shoot, **R** reloads, **L** reopens the lobby panel.

Any world becomes an arena by shipping `arena/arena.json`: score limit, health, respawn time,
match length, the lobby spawn, each map's red and blue spawn points (eye position + yaw) and each
gun's stats (damage, fire rate, magazine, spread, pellets, recoil, headshot multiplier). A gun's
`object` names a model in the level, built with the barrel down -Z. Online, the server is the
referee (`ArenaRules`), so scores and deaths can't be faked by one client.

To watch two clients play each other without touching them, start a server and launch both with
`-Droastengine.arenaAuto=vote0,slot4,ready,aim,shoot`.

## The Backrooms (a world with its own home screen)

**The Backrooms** ([mods/backrooms/](mods/backrooms/), generated by
`tools/generate_backrooms.py`) is a single-player level set: eight levels, an intro video, and
something walking around down there with you.

| Level | What it is |
|---|---|
| **0 - The Lobby** | Yellow wallpaper, damp carpet, the hum. Where everyone starts. |
| **1 - Habitable Zone** | Concrete and steel, pillared halls, cold light. |
| **2 - Pipe Dreams** | Tight, dark, pipes along every ceiling. |
| **3 - Electrical Station** | Thin brick corridors and switchgear cabinets. |
| **4 - Abandoned Office** | Cubicle partitions you can see over, desks, dead monitors. |
| **5 - Terror Hotel** | Faded striped paper, red carpet, a door every few paces. |
| **6 - Lights Out** | No working light at all. Only exit signs, and they lead nowhere. |
| **37 - The Poolrooms** | White tile, ankle-deep water, light from nowhere - and nothing in it. |

The layouts follow the level descriptions on the Backrooms wikis; the Poolrooms being empty is
canon rather than an oversight, so that one is a place to breathe.

**What walks each level.** One creature per level, each the one that belongs there, and each
with its own way of hunting:

| Level | Creature | How it hunts |
|---|---|---|
| Level 0 - The Lobby | **Scrawler** | A person scribbled onto the world and never rubbed out: ragged black, torn red mouth, arms to its shins. |
| Levels 1 and 3 | **Hound** | Not a dog - a human shape on all fours, shaggy hair, a mouth too big for its head. Hears everything, sees little. |
| Levels 2, 5 and 6 | **Smiler** | In the dark it is only a grin and two eyes. Slow, far-sighted, and it does not give up. |
| Level 4 - Abandoned Office | **Faceling** | A person's height, a person's clothes, and a smooth blank where the face should be. |
| Level 37 - The Poolrooms | *nothing* | |

They are built from the engine's own boxes and cylinders in the shape of the entities the
Backrooms wikis describe - the Hound is a crawling humanoid rather than a dog because that is
what the entry says - and not lifted from anyone's artwork. A level names its creature in `backrooms/level.json` under `monster`;
anything it leaves out (speed, hearing, reach) falls back to the world's own `monster` block, so
a level that only wants a faster one says just that.

**The creatures are rigged, and you can animate them.** Each one is a Blender file in
[rigs/backrooms/](rigs/backrooms/) - mesh, armature, materials and starter animations - built by
`tools/make_entity_rigs.py`:

```bash
/Applications/Blender.app/Contents/MacOS/Blender --background --python tools/make_entity_rigs.py
```

Open `rigs/backrooms/smiler.blend`, animate, then **File > Export > glTF 2.0**, format glB, and
under *Animation* set Mode to **Actions** so every action comes out as its own clip. Save over
`mods/backrooms/assets/smiler.glb` and the game picks it up next run. Each body part is bound
rigidly to one bone (weight 1), so poses do exactly what you key with no weight bleeding between
limbs.

The engine chooses the clip from what the creature is doing:

| Clip | When it plays |
|---|---|
| `idle` | standing still |
| `walk` | wandering, or coming to look at a noise |
| `chase` | it can see you |
| `attack` | it has reached you - plays once and holds on the last frame |
| `death` | nothing kills the creature yet, so this one is waiting for a reason to |

A clip the file does not have falls back to the first one in it, so a rig exported with a single
animation still moves. A level points at its rig with `"model": "assets/smiler.glb"` in its
monster block; without one, the static `.obj` version is used instead and simply slides about.

To look at a pose without playing:
`-Droastengine.backroomsAuto=level2,play,skip,pose -Droastengine.backroomsClip=attack
-Droastengine.backroomsClipTime=0.45` stands the creature in front of you, frozen at that moment
of that clip.

Loading the world does not drop you straight in. It opens on the world's **own home screen** -
**Enter**, **Options**, **Leave** - drawn over the level itself, which turns slowly behind the
menu. Enter plays the intro and then puts you in the level. The monster wanders until it hears
you (running is loud, walking carries half as far, standing still is silent) or sees you, and
then it comes. The screen edges redden as it closes in. If it reaches you, that is the run.

**Textures.** The yellow rooms use a real wallpaper sample
([tools/textures/backrooms_wallpaper.png](tools/textures/backrooms_wallpaper.png)); the generator
crops it to a whole number of its own repeats so it tiles without a seam, and builds everything
else - carpet, ceiling tiles, concrete, grime - from noise filtered in the frequency domain,
which tiles for the same reason and has no grain running one way across it. Swap that PNG (or
pass `--wallpaper <file>`) and rerun the generator to re-paper the place. UVs come from world
position, so a pattern runs unbroken from one wall block into the next, and each surface says how
many metres one tile covers (`TEXTURED` in the generator).

**Music.** The mod ships its theme as [audio/theme.mp3](mods/backrooms/audio/theme.mp3), named by
`menu.music` in the level file, and it plays on the home screen and all the way down. Music may be
an `.mp3` or an `.ogg` - mp3 is decoded in pure Java (JLayer), so neither needs anything
installed. With no music file, the level falls back to the engine's own synthesized fluorescent
hum.

**Options** holds sound and effects volume, mouse sensitivity, brightness (the levels are dark),
monsters on or off, how dangerous they are (Calm / Normal / Nightmare) and which level to drop
into. The world's own choices are kept in `config/backrooms.properties`; volume and sensitivity
are the game's usual settings.

**The intro.** Put `BK_INTRO.mp4` in [mods/backrooms/video/](mods/backrooms/video/) and it plays
full screen when you press Enter; any key skips it. Decoding is pure Java (JCodec), so nothing
has to be installed, but that also means **H.264 only** - HEVC/H.265 is skipped rather than
played, and an mp4's AAC soundtrack cannot be decoded, so the video plays silent unless an
`.ogg` of the same name sits beside it. With no video there at all, Enter simply starts the
level. The same player can be used by any world: see `VideoPlayer`.

Any world becomes one of these by shipping `backrooms/level.json`: the home screen's wording, the
intro and music paths, the monster's speed, sight, hearing and reach, and for each level its
origin in the world, its spawn points (with the way you face, chosen down the longest clear run
so you never arrive nose-to-wall) and its **map** - the grid of `#` and `.` the monster walks
on. The generator writes that grid and the level's geometry from the same carve, so a wall on the
map is a wall in the world. Levels sit 400 m apart in one scene, so changing level is a teleport
rather than a load.

To drive it hands-free: `-Droastengine.backroomsAuto=level2,options,play,skip,hunt` picks a
level, opens Options, presses Enter, skips the intro and starts the monster in front of you.

## The spawn gun

Every world puts a **table beside where you start, with a spawn gun on it**. Both are built by
the engine rather than shipped by a mod, so a level made last week gets one too, and no mod can
replace them.

- **E** picks the gun up (it goes in a PlaceHolder API slot like anything else), **Q** puts it
  back down.
- **R**, while holding it, opens the item list: everything in the level, plus every model in
  every enabled mod's `assets/`. Type to search, click a row to put that thing on the floor two
  paces ahead of you, facing you.
- Anything spawned can be picked up again, so a mistake can be carried off rather than left
  standing there.

A model from a mod is loaded the first time you spawn it and kept after that; objects already in
the level cost nothing, since the model is the one the level is drawing. In a normal world the
table stands where you spawn; in a world with levels of its own (the Backrooms) it follows you
into whichever level you enter.

Turn it off with `-Droastengine.noSpawnGun=true`. To look at the list without reaching for the
keyboard: `-Droastengine.spawnMenuDemo=true` puts the gun in hand and opens it.

## The animator (Creator)

Make a project of type **Rig** in the Creator, drop a rigged `.glb` in its `assets/`, and it opens
the animator: the model on the left, its skeleton drawn over it, its bones down the right, and a
timeline along the bottom.

- **Clips already in the model are read in, not just listed.** Opening a `.glb` samples each of
  its animations back into keys, so a walk cycle made in Blender can be retouched here rather
  than only played.
- **Posing** is forward kinematics: pick a bone, turn it on X/Y/Z (and shift it), press **K** to
  key that pose at the playhead. **Del** removes the key, the arrow keys step a frame, **Space**
  plays, and right-drag orbits the model.
- A key stores how far a bone has moved **from its rest pose**, so the numbers read the way an
  animator thinks and a clip written for one rig still means something on another.

Clips save beside the model as `<model>.clips.json` - a small, hand-editable format - and the
game loads them **over** the model's own animations, so an animation can be replaced without
re-exporting from Blender. The Backrooms creatures pick these up automatically; any other code
can with `PoseClipFile.loadBesideModel(...)` and `PoseClipFile.applyTo(...)`.

```json
{ "clips": [ { "name": "idle", "seconds": 3.0,
               "bones": { "head": [ { "t": 0.0, "rot": [0, 0, 0] },
                                    { "t": 1.5, "rot": [6, 18, 0] } ] } } ] }
```

`rot` is degrees about the bone's own axes, `move` (optional) is metres along them, both measured
from rest.

**Not there yet:** IK handles and control shapes - the Rigify way of working, where you drag a
foot and the leg follows. The clip format and bone picking are shaped to take them, but today the
animator is FK only. Blender remains the better place to build a rig; this is for animating one
against the game that plays it.
